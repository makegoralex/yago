...
const pages: Record<string, SeoPage> = {
  ...
  '/tools': {
    ...
    keywords: 'калькуляторы для кофейни, себестоимость кофе, точка безубыточности кофейни, открытие кофейни',
    canonicalPath: '/tools',
    schema: [
      {
        '@context': 'https://schema.org',
    ...
    fallbackHtml: pageShell(
      ...
      'Рассчитайте экономику заведения прямо в браузере — без регистрации и передачи исходных данных.',
      ['Калькулятор себестоимости напитков', 'Калькулятор прибыли и точки безубыточности', 'Калькулятор открытия кофейни']
    ),
  },
