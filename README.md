...
### 2.1. Evotor Android app (MVP)

Android-приложение лежит в папке `android-app`. Чтобы открыть его в Android Studio,
используйте **File → Open** и выберите именно эту папку (не весь репозиторий).

Подробные шаги запуска описаны в `docs/evotor-android.md`.
...
### 3.1. PRINT_JOB_TOKEN для печати чеков

- Сгенерируйте токен локально:
  ```bash
  npm run print:token
  ```
  Скрипт выведет строку вида `PRINT_JOB_TOKEN=...`. Скопируйте значение без
  пробелов.
- На хостинге REG.RU откройте панель управления сайтом/приложением Node.js и
  добавьте переменную окружения `PRINT_JOB_TOKEN` в разделе «Переменные
  окружения» или «Environment variables» (точное название зависит от тарифного
  плана и панели).
- После изменения переменных перезапустите процесс:
  ```bash
  pm2 restart <имя-процесса> --update-env
  ```
  Если процесс запускали через `pm2 start dist/index.js --name yago-api`, то имя
  процесса — `yago-api`.
